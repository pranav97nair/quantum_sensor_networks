...
###
def init_GHZ_programs(num_nodes: int, measure_qubits: bool=True, send_qubits: bool=False, full_state: bool=False):
    node_names = [f"Node_{i+1}" for i in range(num_nodes)]
    if measure_qubits:
        programs = {name: GHZProgram(name, node_names) for name in node_names}
    elif send_qubits:
        programs = {name: GHZProgram_send_qubit(name, node_names) for name in node_names}
    elif full_state:
