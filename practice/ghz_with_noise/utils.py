...
from application import GHZProgram, GHZProgram_send_states
...
###
def init_GHZ_programs(num_nodes: int, measure_qubits: bool=True):
    node_names = [f"Node_{i+1}" for i in range(num_nodes)]
    programs = (
        {name: GHZProgram(name, node_names) for name in node_names}
        if measure_qubits else
        {name: GHZProgram_send_states(name, node_names) for name in node_names}
    )
...
###
def configure_network(node_names: List[str], use_high_fidelity: bool, use_optimistic: bool, link_typ: str):
    # Load generic qdevice configuration from YAML file
...
if __name__ == '__main__':
    ...
    #new_link_fidelity = configure_link(link_typ="perfect").fidelity
    #network_cfg.links[0].cfg.fidelity = new_link_fidelity
    #pprint([link.__dict__ for link in network_cfg.links])