...
class DishonestProgram_zero_rotation(Program):
    ...
    def run(self, context: ProgramContext):
        ...
        if action == "continue":
            ...
            self.target_qubit.rot_Z(angle=0)          
...
class DishonestProgram_Z_measurement(Program):
    def __init__(self, name:str, node_names:List[str], ntest:int, send_state:bool=False):
        self.name = name
        self.node_names = node_names
        self.peer_names = [peer for peer in self.node_names if peer != self.name]
...
class DishonestProgram_incorrect_rotation(Program):
    def __init__(self, name:str, node_names:List[str], ntest:int, send_state:bool=False):
        self.name = name
        self.node_names = node_names
        self.peer_names = [peer for peer in self.node_names if peer != self.name]
    ...
    def run(self, context: ProgramContext):
        ...
        if action == "continue":
            # Encode an incorrect phase onto target qubit
            rot_angle = random.uniform(0, np.pi)
            self.target_qubit.rot_Z(angle=rot_angle)          
