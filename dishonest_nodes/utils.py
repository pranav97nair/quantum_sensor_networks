...
from dishonest_programs import *
...
pwd = '/home/pgnair/stage/dishonest_nodes/'
...
def init_dishonest_programs(num_nodes: int, n_test: int, failure_threshold: float, num_dishonest: int, action: int):
    # Initialize node names list and select verifier node
    node_names = [f"Node_{i+1}" for i in range(num_nodes)]
    verifier = node_names[0]
    ...
    if action == 1:
        programs.update({name: DishonestProgram_zero_rotation(name, node_names, n_test) 
                         for name in dishonest_nodes})
    elif action == 2:
        programs.update({name: DishonestProgram_Z_measurement(name, node_names, n_test) 
                         for name in dishonest_nodes})
    elif action == 3:
        programs.update({name: DishonestProgram_incorrect_rotation(name, node_names, n_test) 
                         for name in dishonest_nodes})
    else:
        raise ValueError("Dishonest action must be between 1 and 3.")
...
if __name__ == '__main__':
    # Test code
    num_nodes = 3
    programs, node_names = init_sensing_programs(num_nodes)
