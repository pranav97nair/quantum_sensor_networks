...
from dishonest_sensing_programs import *
...
pwd = '/home/pgnair/stage/dishonest_new'
...
def init_dishonest_sensing(num_nodes: int, n_test: int, copies: int, failure_threshold: float, num_dishonest: int, action: int):
    # Initialize node names list and select verifier node
    node_names = [f"Node_{i+1}" for i in range(num_nodes)]
    verifier = node_names[0]
    ...
    if action == 1:
        programs.update({name: DishonestSensing_applyZ(name, node_names, n_test, copies) 
                         for name in dishonest_nodes})
    elif action == 2:
        programs.update({name: DishonestSensing_applyX(name, node_names, n_test, copies) 
                         for name in dishonest_nodes})
    else:
        raise ValueError("Dishonest action must be either 1 or 2.")
